...
bool sis_thread_create(SIS_THREAD_START_ROUTINE func, void* var, s_sis_thread_id_t*thread)
{
	uintptr_t result = 0;
	unsigned int id = 0;
	...
	{
		CloseHandle((s_sis_thread_id_t)result);
	}

	*thread = (s_sis_thread_id_t)result;
	return true;
}
