...
int _sisdb_collect_search_from_last(s_sisdb_collect *collect_, uint64 finder_, int *mode_)
{
	...
	for (int i = collect_->value->count - 1; i >= 0 ; i--)
	{
...
int _sisdb_collect_check_lasttime(s_sisdb_collect *collect_, uint64 finder_)
{
	...
	uint64 old_index = sis_dynamic_db_get_mindex(tb->db, 0, sis_struct_list_last(collect_->value), tb->db->size);
...
{
	s_sisdb_table *tb = collect_->sdb;
	int start = collect_->value->count - 1;
	if (index_ >= 0)
	{
		start = index_;
	}
	for (int i = start; i >= 0 ; i--)
	{
		...
		if (mindex_ > 0)
		{
			...
			if (mindex < mindex_)
			{
				...
				{
					sis_struct_list_insert(collect_->value, index_ + 1, (void *)in_);
				}
				return 1;
			}
		}
