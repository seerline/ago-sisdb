#ifndef _SIS_DISK_V1_H
#define _SIS_DISK_V1_H
...
typedef struct s_sis_disk_v1_main_head {
    ...
    uint16   other;               // 
}s_sis_disk_v1_main_head;            // 16个字节头 128
...
#define  SIS_DISK_V1_MIN_BUFFER   sizeof(s_sis_disk_v1_main_tail)
...
typedef struct s_sis_disk_v1_callback {
...
}s_sis_disk_v1_callback;

//////////////////////////////////////////////////////
// 字典表 多数文件的信息都从这里找到对应关系
//////////////////////////////////////////////////////
typedef struct s_sis_disk_v1_dict_unit {
    uint8              writed; // 是否已经写盘
	s_sis_dynamic_db  *db;     // 如果是数据结构 表示结构的长度 若没有为NULL
}s_sis_disk_v1_dict_unit;

// key 和 sdb 的基本信息
typedef struct s_sis_disk_v1_dict {
    int                 index;  // 索引号
...
typedef struct s_sis_disk_v1_index_unit
{
...
} s_sis_disk_v1_index_unit;

#define SIS_DISK_IDX_TYPE_NONE  '0'
#define SIS_DISK_IDX_TYPE_INT   SIS_DYNAMIC_TYPE_INT
#define SIS_DISK_IDX_TYPE_MSEC  SIS_DYNAMIC_TYPE_MSEC  
#define SIS_DISK_IDX_TYPE_SEC   SIS_DYNAMIC_TYPE_TSEC   
...
typedef struct s_sis_disk_v1_index {
    uint32              cursor;    // 当前读到第几条记录
    s_sis_object       *key;       // 可能多次引用 - 指向dict表 因为可能文件没有索引
    s_sis_object       *sdb;       // 可能多次引用 - 指向dict表
    s_sis_struct_list  *index;     // 索引的列表 s_sis_disk_v1_index_unit
}s_sis_disk_v1_index;

// 专门用来缓存sno数据的
typedef struct s_sis_disk_v1_rcatch {
    int                rsize;    // 当前的记录长度
    s_sis_disk_v1_dict   *kdict;
    s_sis_disk_v1_dict   *sdict;
    s_sis_memory      *omem;    // 数据缓存指针 可能多条记录 已经移动到数据区
}s_sis_disk_v1_rcatch;

typedef struct s_sis_disk_v1_rsno_unit {
    s_sis_object      *keyn;     // 可能多次引用 - 指向dict表 因为可能文件没有索引
    s_sis_object      *sdbn;     // 可能多次引用 - 指向dict表
    int                size;     // 当前的数据长度
    char*              optr;     // 指向omem数据指针
}s_sis_disk_v1_rsno_unit;

typedef struct s_sis_disk_v1_rsno {
    uint64                sno;     // 用来排序的标志号
    s_sis_disk_v1_rsno_unit *unit;    
}s_sis_disk_v1_rsno;
// 通用的读取数据函数 如果有字典数据key就用字典的信息 
// 如果没有字典信息就自己建立一个
typedef struct s_sis_disk_v1_wcatch {
    s_sis_object         *key;      // 可能多次引用 - 指向dict表 因为可能文件没有索引
    s_sis_object         *sdb;      // 可能多次引用 - 指向dict表
    s_sis_memory         *memory;   // 可能数据缓存指针
    s_sis_disk_v1_index_unit winfo;    // 写入时的索引信息
}s_sis_disk_v1_wcatch;


typedef struct s_sis_disk_v1_reader {
    uint8                issub;
    uint8                isone;   // 是否一次性输出
////////// ask  //////////
    ...
    s_sis_disk_v1_callback *callback;  // 读文件的回调
...
typedef struct s_sis_disk_v1_unit {
...
// 多文件管理类
typedef struct s_sis_disk_v1 {
    uint8                 access;     // 只有三种状态
    s_sis_memory         *zip_memory;     // 压缩用缓存区
    s_sis_disk_v1_main_head  main_head;      // 文件头
    s_sis_disk_v1_main_tail  main_tail;      // 文件尾
	...
	s_sis_pointer_list   *lists;          // s_sis_disk_v1_unit 文件信息 有几个就打开几个
} s_sis_disk_v1;

typedef struct s_sis_disk_v1_class {
    s_sis_mutex_t        mutex;  // 多线程写入时需要加锁，不然报错
    bool                 isinit; // 必须初始化后才能使用
    // uint8                isstop;  // 读文件期间是否停止 如果为 1 立即退出
    bool                 isstop; // 是否中断读取
    s_sis_disk_v1_reader   *reader; // 读取数据时保存的指针
	...
	s_sis_map_list      *keys;   // s_sis_disk_v1_dict
	s_sis_map_list      *sdbs;   // s_sis_disk_v1_dict

    s_sis_disk_v1_wcatch  *src_wcatch;     // 需要转入公共区的 wcatch
  
    s_sis_map_list     *index_infos;      // 以key.sdb 为索引的 s_sis_struct_list 列表 存储 s_sis_disk_v1_index

    size_t              sno_size;       // SIS_DISK_HID_MSG_SNO 当前累计数据量
    size_t              sno_series;     // SIS_DISK_HID_MSG_SNO 当前最新计数
    uint32              sno_pages;      // 页面计数
    s_sis_map_list     *sno_wcatch;     // 从实时文件转盘后文件 临时存放数据 s_sis_disk_v1_wcatch
    s_sis_pointer_list *sno_rcatch;     // 分块读取时的缓存 s_sis_disk_v1_rcatch 需要根据sno进行快速排序

    s_sis_disk_v1        *work_fps;      // 工作文件组
    s_sis_disk_v1        *index_fps;     // 索引文件组
...
s_sis_disk_v1_dict_unit *sis_disk_v1_dict_last(s_sis_disk_v1_dict *);
s_sis_disk_v1_dict_unit *sis_disk_v1_dict_get(s_sis_disk_v1_dict *, int);
// 传递一个 dict 单元 返回值 = 0 表示没有新增 1.n表示新增
int sis_disk_v1_dict_set(s_sis_disk_v1_dict *, bool iswrite_, s_sis_dynamic_db * db_); 

///////////////////////////
//  s_sis_disk_v1_rcatch
///////////////////////////
// 先把磁盘读取的数据 保存在列表中
s_sis_disk_v1_rcatch *sis_disk_v1_rcatch_create(int size_, s_sis_memory *omem_); 
...
//  s_sis_disk_v1_reader
///////////////////////////
s_sis_disk_v1_reader *sis_disk_v1_reader_create(s_sis_disk_v1_callback *); 
void sis_disk_v1_reader_destroy(void *in_);
void sis_disk_v1_reader_clear(s_sis_disk_v1_reader *in_);
void sis_disk_v1_reader_set_key(s_sis_disk_v1_reader *in_, const char *);
void sis_disk_v1_reader_set_sdb(s_sis_disk_v1_reader *in_, const char *);
...
void sis_disk_v1_destroy(void *);

void sis_disk_v1_clear(s_sis_disk_v1 *);
...
// // 移动指针到第几个文件 某个位置
// long long sis_disk_v1_seek(s_sis_disk_v1 *, int fidx_, size_t offset_, int where_);
// // 从当前位置读取数据

// 回调返回为 -1 表示已经没有读者了,停止继续读文件
typedef int (cb_sis_disk_v1_read)(void *, s_sis_disk_v1_head *, s_sis_memory *);
// 全文读取 数据后通过回调输出数据
size_t sis_disk_v1_read_fulltext(s_sis_disk_v1 *cls_, void *, cb_sis_disk_v1_read *callback);
// 定位读去某一个块
size_t sis_disk_v1_read(s_sis_disk_v1 *, int fidx_, size_t offset_, size_t size_, 
...
// 设置文件大小
void sis_disk_v1_class_set_size(s_sis_disk_v1_class *,size_t fsize_, size_t psize_);
...
// list_ 为 s_sis_disk_v1_index 结构队列 其中 index 仅仅为指向索引的指针 销毁时要注意
...
// 加载索引文件到内存 方便检索
int cb_sis_disk_v1_file_read_index(void *cls_, s_sis_disk_v1_head *, s_sis_memory *);
