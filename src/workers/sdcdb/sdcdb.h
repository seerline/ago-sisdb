...
typedef struct s_sdcdb_reader
{
	...
	void               *sub_disker;   // 从从盘读取数据的对象  s_sdcdb_disk_worker
	...
	s_sdcdb_worker     *sub_ziper;     // 用于压缩的操作类
	s_sdcdb_worker     *sub_unziper;   // 用于解压的操作类
	///////以上为定制数据处理///////
...
typedef struct s_sdcdb_cxt
{
	...
	s_sdcdb_reader     *wlog_reader;         // 写wlog的读者
...
bool  sdcdb_init(void *, void *);
void  sdcdb_uninit(void *);
void  sdcdb_method_init(void *);
void  sdcdb_method_uninit(void *);

// ***  数据流操作 ***//
// 数据采用结构化数据压缩方式，实时压缩，放入无锁发送队列中 
// 每10ms打一个包或者数据量超过16K(减少拆包和拼包的动作)一个包,
// 为相关压缩必须保证包的顺序 当时间为整10分钟时重新压缩 保证最差能取到最近 10 分钟数据
// sdcdb 仅仅作为一个数据组件使用 和网络无关
// *****************//
...
void sdcdb_worker_destroy(s_sdcdb_worker *);

void sdcdb_worker_clear(s_sdcdb_worker *);

void sdcdb_worker_set_keys(s_sdcdb_worker *, s_sis_sds );
...
void sdcdb_worker_zip_set(s_sdcdb_worker *, int ,int, char *, size_t);
...
// 清理指定的 reader
int sdcdb_move_reader(s_sdcdb_cxt *, int);
