...
void sis_disk_reader_set_time(s_sis_disk_reader *cls_, msec_t start_, msec_t stop_)
{
    cls_->start = start_;
    cls_->stop = stop_;
}
...
int sis_reader_sub_filters(s_sis_disk_class *cls_, s_sis_disk_reader *reader_, s_sis_pointer_list *list_)
{
    ...
    s_sis_string_list *keys = sis_string_list_create_w();
    ...
    int issdbs = 0;
    s_sis_string_list *sdbs = sis_string_list_create_w();
...
size_t cb_sis_disk_file_read_work(void *source_, s_sis_disk_head *head_, s_sis_object *obj_)
{
    ...
    //     _sis_disk_read_hid_sdb(cls_, obj_);
    //     break;
    // case SIS_DISK_HID_MSG_KDB:
    //     _sis_disk_read_hid_kdb(cls_, obj_);
    //     break;
    // case SIS_DISK_HID_MSG_KEY:
    //     _sis_disk_read_hid_key(cls_, obj_);
    //     break;
    // case SIS_DISK_HID_MSG_ANY:
    //     _sis_disk_read_hid_any(cls_, obj_);
    //     break;
...
{
    if (cls_->sno_rcatch->count > 0 ) // 数据读完了
    {
        // 一个块数据读完 对数据进行排序 
        // 指针列表排序 必须直接传入buffer
...
size_t cb_sis_disk_file_read_sno(void *source_, s_sis_disk_head *head_, s_sis_object *obj_)
{
    s_sis_disk_class *cls_ = (s_sis_disk_class *)source_;
    s_sis_disk_callback *callback = cls_->reader->callback; 
    // 根据hid不同写入不同的数据到obj
...
int sis_disk_read_sub_sdb(s_sis_disk_class *cls_, s_sis_disk_reader *reader_)
{
    ...
    {
        bool iswhole = true;
        if (reader_->stop > reader_->start || (reader_->stop == 0 && reader_->start == 0))
        {
            iswhole = false;
        }
        s_sis_disk_index *idxinfo = (s_sis_disk_index *)sis_pointer_list_get(filters, i);
        if (idxinfo)
        {
            for (int k = 0; k < idxinfo->index->count; k++)
            {
                ...
                if (iswhole || (unit->stop == 0 && unit->start == 0) ||
                    sis_is_mixed(reader_->start, reader_->stop, unit->start, unit->stop) )
                {
                    uint8 hid = 0;
...
int sis_disk_read_get_sdb(s_sis_disk_class *cls_, s_sis_disk_reader *reader_)
{
    ...
    // 如果严格卡时间可以在这里做过滤
    if (sis_memory_get_size(SIS_OBJ_MEMORY(obj)) > 0 && reader_->callback && reader_->callback->cb_read)
    {
...
size_t cb_sis_disk_file_read_index(void *source_, s_sis_disk_head *head_, s_sis_object *obj_)
{
    s_sis_disk_class *cls_ = (s_sis_disk_class *)source_;
    // 直接写到内存中
    int  maxpages = 0;
    ...
    while(sis_memory_get_size(memory) > 0)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < blocks; i++)
            {
                ...
                unit.kdict = sis_memory_get_ssize(memory);
                unit.sdict = sis_memory_get_ssize(memory);
                unit.fidx = sis_memory_get_ssize(memory);
                unit.offset = sis_memory_get_ssize(memory);
                unit.size = sis_memory_get_ssize(memory);
...
int sis_disk_file_read_sub(s_sis_disk_class *cls_, s_sis_disk_reader *reader_)
{
    ...
    case SIS_DISK_TYPE_STREAM:
        // 每次都从头读起
        sis_files_read_fulltext(cls_->work_fps, cls_, cb_sis_disk_file_read_work);
        break;
...
int sis_disk_file_read_get(s_sis_disk_class *cls_, s_sis_disk_reader *reader_)
{
    ...
    sis_disk_read_get_sdb(cls_, reader_);
