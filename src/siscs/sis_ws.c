...
void after_read(uv_stream_t* handle, ssize_t nread, uv_buf_t buf) {
  ...
  if(ctx->request->handshake == 0) {
    ...
    printf("------http read %d\n", ctx->request->handshake);
    sis_out_binary("", buf.base,nread);
    ...
    size_t np = ws_execute_out(ctx->wsparser, &wssettings, buf.base, 0, nread);
    // ???????????????????
    s_sis_sds reply = siscs_send_local_sds("sisdb.get","sh600600.day", NULL, 0);

    if (!reply) return ;
    ///////////////////////////////////
    sis_out_binary("", reply,64);

    write_req_t *wr;
    wr = (write_req_t*) malloc(sizeof *wr);
    // size_t sendlen = sis_sdslen(reply) + 

    char *buffer = malloc(nread);
    buffer[0]=buf.base[0];
    buffer[1]=buf.base[1] & 0x7F;

    memmove(&buffer[2], reply, nread -4);
    // wr->buf = uv_buf_init(buf.base, nread);
    wr->buf = uv_buf_init(buffer, nread - 4);
    printf("------write \n");
    sis_out_binary("", wr->buf.base,wr->buf.len);
    if (uv_write(&wr->req, ctx->handle, &wr->buf, 1, after_write)) {
    // if (uv_write(&wr->req, ctx->handle, &wr->buf, 1, NULL)) {
      exit(1);
    }
    sis_sdsfree(reply);
    ...
    // memmove(&buffer[2], &buf.base[6],nread -4);
    // // wr->buf = uv_buf_init(buf.base, nread);
    // wr->buf = uv_buf_init(buffer, nread - 4);
    // printf("------write \n");
    // sis_out_binary("", wr->buf.base,wr->buf.len);
