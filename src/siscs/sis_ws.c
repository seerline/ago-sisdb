...
void cb_write_after_hand(uv_write_t *write, int status)
{
  if (status < 0)
  {
    uv_err_t err = uv_last_error(__server.loop);
...
void cb_read_after(uv_stream_t *handle, ssize_t insize, uv_buf_t buffer)
{
  ...
  {
    size_t bytes = sis_http_parser_execute(client->parser, &__http_settings, buffer.base, insize);
    
    sis_out_binary("hand", buffer.base, insize);
  ...
  else
  {
    ...
    sis_out_binary("source: ", buffer.base, insize);

    sis_ws_messages_input(client->messages, handle, buffer.base, insize);
...
void cb_connection(uv_stream_t *server, int status)
{
  ...
  if (status != 0)
  {
    LOG(1)("connect error %d\n", uv_last_error(__server.loop));
  }
  assert(status == 0);
...
int sis_ws_server_start(int port)
{
  ...
  r = uv_tcp_bind(&__server.server, address);
  if (r)
  {
    LOG(1)
    ("bind error. [%d]\n", port);
