...
#define STS_INSERT_TS_CHECK    1  // 检查时间节点重复就覆盖老的数据，不重复就放对应位置
...
typedef struct sts_table{
	sds name;            //表的名字
	sts_table_control control;       // 表控制定义
	s_string_list  *field_name;      // 按顺序排的名字
	s_map_pointer  *field_map;       // 字段定义字典表，按字段名存储的字段内存块，指向sts_field_unit
	s_map_pointer  *collect_map;     // 数据定义字典表，按股票名存储的数据内存块，指向sts_collect_unit
}sts_table;

#pragma pack(pop)

sts_table *create_sts_table(const char *name_, const char *command);  //command为一个json格式字段定义
// command为json命令
...
void destroy_sts_table(sts_table *);  //删除一个表
void clear_sts_table(sts_table *);    //清理一个表的所有数据
//对数据库的各种属性设置
void sts_table_set_ver(sts_table *, uint32_t);  // time_t格式
void sts_table_set_limit_rows(sts_table *, uint32_t); // 0 -- 不限制  1 -- 只保留最新的一条  n 
void sts_table_set_zip_mode(sts_table *, uint8_t); // 0 -- 不压缩 1 2
void sts_table_set_insert_mode(sts_table *, uint8_t); // 1 -- 判断后修改 0 2

void sts_table_set_fields(sts_table *, const char *command); //command为一个json格式字段定义
//获取数据库的各种值
sts_field_unit *sts_table_get_field(sts_table *tb_, const char *name_);
int sts_table_get_fields_size(sts_table *);
uint64 sts_table_get_times(sts_table *, void *); // 获取时间序列,默认为第一个字段，若第一个字段不符合标准，往下找
...
sds sts_table_get_m(sts_table *, const char *key_, const char *command);  //返回数据需要释放
...
int sts_table_delete(sts_table *, const char *key_, const char *command);// command为json命令
//删除
//用户传入的command中关键字的定义如下：
//数据范围："search":   min,max 按时序索引取数据
//						count(和max互斥，正表示向后，负表示向前),
