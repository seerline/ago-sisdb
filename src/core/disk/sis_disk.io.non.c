...
size_t _disk_io_write_non_work(s_sis_disk_ctrl *cls_, s_sis_disk_wcatch *wcatch_)
{ 
...
size_t _disk_io_write_non_widx(s_sis_disk_ctrl *cls_, s_sis_disk_wcatch *wcatch_)
{ 
    if (wcatch_->head.zip == SIS_DISK_ZIP_SNAPPY)
    {
        s_sis_memory *zipmem = sis_memory_create();
