...
size_t _disk_io_write_sno_and_idx(s_sis_disk_ctrl *cls_, s_sis_disk_wcatch *wcatch_)
{ 
    size_t size = sis_disk_files_write_saveidx(cls_->work_fps, wcatch_); 
    if (wcatch->head.hid == SIS_DISK_HID_SNO_END)
    {
        s_sis_disk_idx *node = sis_disk_index_get(cls_->map_idxs, SIS_DISK_SIGN_SNO, NULL);
...
int cb_incrzip_encode_sno(void *source, char *in, size_t ilen)
{
    ...
    size_t size = _disk_io_write_sno_and_idx(ctrl, wcatch);
...
int sis_disk_io_write_sno_stop(s_sis_disk_ctrl *cls_)
{
    ...
    sis_incrzip_compress_destroy(cls_->sno_incrzip);
...
{
    s_sis_dynamic_db *sdb = sis_disk_dict_last(sdict);
    ...
    if (cls_->sno_zipsize > cls_->work_fps->max_page_size)
    {
        ...
        _disk_io_write_sno_and_idx(cls_, wcatch);
        sis_disk_wcatch_init(wcatch);
        cls_->sno_zipsize = 0;
...
size_t sis_disk_io_write_snoidx(s_sis_disk_ctrl *cls_)
{
    ...
    if (snonode)
    {
        sis_memory_cat_ssize(memory, snonode->index->count);
        for (int k = 0; k < snonode->index->count; k++)
        {
...
static int cb_incrzip_decode_sno(void *source_, int kidx, int sidx, char *in, size_t ilen)
{
    ...
    // 这里可以做过滤
    const char *kname = sis_disk_kdict_get_name(ctrl->map_kdicts, kidx);
    const char *sname = sis_disk_kdict_get_name(ctrl->map_sdicts, sidx);
    if (ctrl->reader->iswhole || (kname && sname &&
        !sis_str_subcmp(kname, ctrl->reader->sub_keys, ',') >= 0 &&
        !sis_str_subcmp(sname, ctrl->reader->sub_sdbs, ',') >= 0))
    {
        ...
        if (callback->cb_userdate)
        {
            ...
            callback->cb_userdate(callback->cb_source, kname, sname, in, ilen);     
        }
    }
    return 0;
}
...
int cb_sis_disk_io_read_sno(void *source_, s_sis_disk_head *head_, char *imem_, size_t isize_)
{
    ...
    case SIS_DISK_HID_DICT_KEY:
        ...
        {
            callback->cb_dict_keys(callback->source, imem_, isize_);
        }
        break;
    case SIS_DISK_HID_DICT_SDB:
        if(callback && callback->cb_key)
        {
            callback->cb_dict_sdbs(callback->source, imem_, isize_);
        }
        break;
    default:
...
int cb_sis_disk_io_read_snoidx(void *source_, s_sis_disk_head *head_, char *imem_, size_t isize_)
{
...
int sis_disk_io_read_snoidx(s_sis_disk_ctrl *cls_)
{
    if (cls_->work_fps->main_head.index)
    {
        ...
        sis_disk_files_read_fulltext(cls_->widx_fps, cls_, cb_sis_disk_io_read_snoidx);
