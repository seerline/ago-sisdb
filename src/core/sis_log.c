...
void sis_log_check()
{
	...
	if (filesize > (_sis_log.maxsize * 1024 * 1024))
	{
		...
		_sis_log.logfp = sis_file_open(_sis_log.filename, 
			// SIS_FILE_IO_READ | SIS_FILE_IO_WRITE | SIS_FILE_IO_TRUCT, 0);
			SIS_FILE_IO_READ | SIS_FILE_IO_WRITE | SIS_FILE_IO_CREATE, 0);
	}
...
{
	sis_file_handle fp = sis_file_open(name, SIS_FILE_IO_READ | SIS_FILE_IO_WRITE | SIS_FILE_IO_CREATE, 0);
	if (fp)
	{
		size_t size = sis_file_write(fp, (char *)value, 1, len);
		sis_file_close(fp);
		return size;
	}
	return 0;
