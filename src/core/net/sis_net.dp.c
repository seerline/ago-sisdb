...
#include <sis_net.dp.h>
#include <sis_math.h>

// ***************************************************** //
// 接收端 需要事先申请好 内存队列 根据流量调整队列大小 可缩小可放大
//       收到数据后放入每个用户的内存队列中 然后再拼装解析数据 完整数据传给上层 上层最后生成netmsg
//       解析完的后队列放入末尾循环使用 避免多次内存拷贝
// 发送端 要求支持多用户发送数据不干扰
//       支持同一份数据引用发送 
//       客户发送netmsg给上层，上层生成网络序列化的二进制数据流 传入每个客户的发送队列中
//       当该socket可写时 就从发送队列获取数据写入 如果发送对端速度极慢 就直接断开链接 并清理该用户的缓存 
// 支持各种转发
// ***************************************************** //
