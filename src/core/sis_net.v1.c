...
s_sis_socket *sis_socket_create(s_sis_url_v1 *url_,int rale_)
{

	s_sis_socket *sock = sis_malloc(sizeof(s_sis_socket));
	memmove(&sock->url, url_, sizeof(s_sis_url_v1));
	sock->rale = rale_;
	
	sock->message_send = sis_list_create();
	sock->message_send->free = sis_net_message_destroy;

	sock->message_recv = sis_list_create();
	sock->message_recv->free = sis_net_message_destroy;

	sock->sub_keys = sis_list_create();
	sock->sub_keys->free = sis_net_message_destroy;
	sock->pub_keys = sis_list_create();
	sock->pub_keys->free = sis_net_message_destroy;
...
void sis_socket_destroy(void *sock_)
{
	...
	sis_list_destroy(sock->message_send);
	sis_list_destroy(sock->message_recv);
	sis_list_destroy(sock->sub_keys);
	sis_list_destroy(sock->pub_keys);
