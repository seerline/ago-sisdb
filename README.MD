...
若已经安装了网络和其他基础组件可以直接跳转第二步：
   ...
   ***虚拟机的Network配置要选择 Bridged Adapter.
   ...
   firewall-cmd --reload
   cd bin
   ./sisdb-server siserver.conf
      *** 开启本机或其他的终端
   ./sisdb-cli -a clxx1110
   ./sisdb-cli -a clxx1110 -h 192.168.xxx.xxx
...
## 配置说明
   ...
   > append-method: 
      生成新记录时需要检查的字段；
      总共3种方法名 ： 
         incr 参数为字段列表，表示所有字段必须都在增长才能增加记录，
         nonzero 参数为字段列表，表示需要不等于0
         together 参数为字段列表，表示字段没有出现，就增加记录，多个字段时代表多个字段都未出现时
         多个方法之间关系为并且的关系，
   > limit:
      最大保存记录数
