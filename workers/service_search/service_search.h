#ifndef _SERVICE_SEARCH_H
#define _SERVICE_SEARCH_H
...
typedef struct s_service_search_reader
{
	s_sis_sds             sub_mkey; // 匹配字符
	s_sis_pointer_list   *netmsgs;    // s_sis_net_message
} s_service_search_reader;

typedef struct s_service_search_cxt
{
	int                 status;      // 工作状态

	s_sis_sds           dbname;      // 数据库名字 service_search
	...
	s_sis_pointer_list  *msub_readers; // s_service_search_reader 
	// 以 key 为索引的 s_service_search_reader 同一key可能有多个用户订阅
	s_sis_map_pointer   *sub_readers;   // s_service_search_reader

	// 直接回调组装好的 s_sis_net_message
	void               *cb_source;       // 
	sis_method_define  *cb_net_message;  // s_sis_net_message 

}s_service_search_cxt;

s_service_search_reader *service_search_reader_create(s_sis_net_message *netmsg_);
void service_search_reader_destroy(void *);

bool  service_search_init(void *, void *);
void  service_search_uninit(void *);
void  service_search_working(void *);  
