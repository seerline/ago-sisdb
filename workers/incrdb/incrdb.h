...
#define INCRDB_DISK_INIT 0
#define INCRDB_DISK_WORK 1
#define INCRDB_DISK_EXIT 2

#pragma pack(push,1)

// ??? 未生效

typedef struct s_incrdb_bytes
{
	uint32  kidx;
	uint16  sidx;
	uint16  size;
...
// 压缩数据结构
typedef struct s_incrdb_compress
{
	uint8   init;
	uint32  size;
	uint8   data[0];    
...
typedef struct s_incrdb_worker
{
	void                 *cb_source;   // 返回对象 unincrdb专用
	cb_sis_struct_decode *cb_decode;   // 按结构返回数据 unincrdb专用

	s_incrdb_compress     *zip_bits;    // incrdb 专用
...
typedef struct s_incrdb_reader
{
	int        cid;
	s_sis_sds  serial;
	// 以下两个回调互斥
	void      *incrdb_worker;          // 来源对象
	...
	void               *sub_disker;   // 从从盘读取数据的对象  s_incrdb_disk_worker
	...
	s_incrdb_worker     *sub_ziper;     // 用于压缩的操作类
	s_incrdb_worker     *sub_unziper;   // 用于解压的操作类
	///////以上为定制数据处理///////
	...
	// 返回压缩的数据 
	sis_method_define  *cb_sub_inctzip;      // s_incrdb_compress
...
typedef struct s_incrdb_disk_worker
{
	int                rdisk_status;  // 读取磁盘的状态 0 初始或退出完成 1 正常 2 请求中断退出
	s_sis_worker      *rdisk_worker;  // 读文件类
	s_incrdb_reader    *incrdb_reader;  // 仅仅是指针
} s_incrdb_disk_worker;

// 来源数据只管写盘 只有key, sdb有的才会保存数据
// 读取数据的人会接收到压缩的 out_bitzips 如果接收者订阅了全部就直接发送数据出去
//          否则就把数据解压，然后写入自己的 s_incrdb_cxt 再通过读者回调发送数据

#define MAP_INCRDB_BITS(v) ((s_incrdb_compress *)sis_memory(v->ptr))

typedef struct s_incrdb_cxt
{
	...
	s_incrdb_reader     *wlog_reader;         // 写wlog的读者
	...
	s_sis_fast_queue   *inputs;    // 传入的数据链 s_incrdb_compress

	s_sis_bits_stream_v0  *cur_sbits;   // 当前指向缓存的位操作类

	s_sis_object       *cur_object;   // s_incrdb_compress -> 映射为memory 当前用于写数据的缓存 
	s_sis_object       *last_object;  // 最近一个其实数据包的指针
	// 这个outputs需要设置为无限容量
	int                 zipnums;    // 统计数量
	s_sis_lock_list    *outputs;    // 输出的数据链 s_incrdb_compress -> 映射为 memory 每10分钟一个新的压缩数据块
	s_sis_pointer_list *readeres;   // 读者列表 s_incrdb_reader
...
bool  incrdb_init(void *, void *);
void  incrdb_uninit(void *);
void  incrdb_method_init(void *);
void  incrdb_method_uninit(void *);

// ***  数据流操作 ***//
// 数据采用结构化数据压缩方式，实时压缩，放入无锁发送队列中 
// 每10ms打一个包或者数据量超过16K(减少拆包和拼包的动作)一个包,
// 为相关压缩必须保证包的顺序 当时间为整10分钟时重新压缩 保证最差能取到最近 10 分钟数据
// incrdb 仅仅作为一个数据组件使用 和网络无关
// *****************//
...
int cmd_incrdb_ipub(void *worker_, void *argv_);  // s_incrdb_bytes
// 需要传入 key sdb val 收到数据后压缩存储
int cmd_incrdb_spub(void *worker_, void *argv_);  // s_sis_db_chars
// 需要传入 zipval 直接是压缩数据块 直接放入队列中
int cmd_incrdb_zpub(void *worker_, void *argv_);  // s_incrdb_compress
// 对指定的无锁队列增加订阅者
...
void incrdb_worker_destroy(s_incrdb_worker *);

void incrdb_worker_clear(s_incrdb_worker *);

void incrdb_worker_set_keys(s_incrdb_worker *, s_sis_sds );
void incrdb_worker_set_sdbs(s_incrdb_worker *, s_sis_sds );
// 写入需要解压的数据
void incrdb_worker_unzip_init(s_incrdb_worker *, void *cb_source, cb_sis_struct_decode *cb_read_);
void incrdb_worker_unzip_set(s_incrdb_worker *, s_incrdb_compress *);
// 写入需要压缩的数据
void incrdb_worker_zip_init(s_incrdb_worker *, int, int);
void incrdb_worker_zip_flush(s_incrdb_worker *, int);
void incrdb_worker_zip_set(s_incrdb_worker *, int ,int, char *, size_t);

//////////////////////////////////////////////////////////////////
//------------------------s_incrdb_reader -----------------------//
//////////////////////////////////////////////////////////////////
int incrdb_sub(s_sis_worker *worker, s_sis_net_message *netmsg, bool iszip);

s_incrdb_reader *incrdb_reader_create();
void incrdb_reader_destroy(void *);

int incrdb_reader_realtime_start(s_incrdb_reader *reader);
int incrdb_reader_realtime_stop(s_incrdb_reader *reader);

int incrdb_reader_new_realtime(s_incrdb_reader *reader_);
// 清理指定的 reader
int incrdb_move_reader(s_incrdb_cxt *, int);
...
#define INCRDB_FILE_SIGN_ZPUB  0  // zpub ...
#define INCRDB_FILE_SIGN_KEYS  1  // zset _sdbs_ ...
#define INCRDB_FILE_SIGN_SDBS  2  // zset _keys_ ...

int incrdb_wlog_load(s_incrdb_cxt *);
// 停止wlog文件
int incrdb_wlog_start(s_incrdb_cxt *);
// 把数据写入到wlog中
int incrdb_wlog_save(s_incrdb_cxt *, int , s_incrdb_compress *);
// 停止wlog文件
int incrdb_wlog_stop(s_incrdb_cxt *);
// 清除wlog文件
int incrdb_wlog_move(s_incrdb_cxt *);
// 把wlog转为snos格式 
int incrdb_wlog_save_snos(s_incrdb_cxt *);
// 读取 snos 文件 snos 为incrdb压缩的分块式顺序格式
s_incrdb_disk_worker *incrdb_snos_read_start(s_sis_json_node *, s_incrdb_reader *);
// 读取 snos 文件 snos 为incrdb压缩的分块式顺序格式
int incrdb_snos_read_stop(s_incrdb_disk_worker *);
